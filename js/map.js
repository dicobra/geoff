...
function arrayBufferToBase64ImagePNG(buffer) {
    ...
    return 'data:image/png;base64,' + btoa(binary);
        ...
        var listavar = document.getElementById('lista');
        var positionvar = document.getElementById('position');
        var onSuccess = function(position) {
            var tempo = new Date(position.timestamp);
        positionvar.innerHTML = '<div class="position">' + '<br>' +
              'Latitude: '          + position.coords.latitude          + '<br/>' +
              'Longitude: '         + position.coords.longitude         + '<br/>' +
              'Accuracy: '          + position.coords.accuracy          + '<br/>' +
              'Timestamp: '         + tempo.getDate() + '/' + tempo.getMonth() + '/' +
              tempo.getFullYear() + ' - ' + tempo.getHours() + ':' + tempo.getMinutes() +
...
map.addControl(new Control({position: 'topleft', innerHTML: 'D', handler: function () {
    PouchDB.destroy(dbname, function (err, value) {
        ...
        function markacao(){
        document.getElementById('lista').innerHTML += document.getElementById('position').innerHTML;
       };